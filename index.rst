...
この演習では Prism Central v3 APIs を使ってみることで、NutanixのAPIに慣れる事ができます。
...
- Prismのイメージサービスを使ってWindowsとLinuxのイメージをアップロードしてください
 - CentOS7.qcow2[http://10.42.194.11/workshop_staging/CentOS7.qcow2](VPN GW Phoenix(ST))
 - Windows2016.qcow2[http://10.42.194.11/workshop_staging/Windows2016.qcow2](VPN GW Phoenix(ST))
- Postmanというツールのアカウントを取得してください

演習環境について
+++++++++++++++++++

この演習(Nutanix Workshops)は Nutanix社の Hosted POC (検証用の環境)を使っておこなわれます。
...
マーケティング用のPOC環境との場合は以下のクラスタが準備されています:
...
各クラスターはそれぞれ仮想マシンのドメインコントローラー **DC** を持っており、それは **NTNXLAB.local** というドメインとしてAD機能を提供しています。
